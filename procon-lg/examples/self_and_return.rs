...
impl TestStruct {
    ...
    fn recursive_countdown(#[show] &self, #[show] count: u32) {
        if count > 0 {
            eprintln!("count = {count}, value = {}", self.value);
...
fn main() {
    ...
    eprintln!("=== Testing method calls ===");
    let result1 = ts.method_ref(10);
    eprintln!("method_ref result: {result1}");

    let old_value = ts.method_mut_ref(100);
    eprintln!("method_mut_ref old_value: {old_value}");

    let result2 = TestStruct::new(5).method_owned(3);
    eprintln!("method_owned result: {result2}");
    ...
    eprintln!("Final result: {result}, x: {x}, y: {y}, z: {z}");
}
